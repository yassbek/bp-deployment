...
export default function CompletionPage() {
  ...
  const applicationSteps = [
    ...
    { title: "Impact-Reife", icon: Target, status: "next", path: `/preparation_impact?applicationId=${applicationId}` },
    { title: "Marketing & Positionierung", icon: Megaphone, status: "pending", path: `/preparation_marketing?applicationId=${applicationId}` },
  ...
  const completedStep = applicationSteps.find((s) => s.status === "completed")
  const nextStep = applicationSteps.find((s) => s.status === "next")
    ...
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-5">
              ...
              <div>
                <h1 className="text-2xl font-bold text-gray-900">Meilenstein erreicht!</h1>
                <p className="text-gray-600">Dein {completedStep?.title}-Interview ist abgeschlossen.</p>
              </div>
            </div>
            <Badge variant="outline" className="border-brand text-brand">
              Schritt 1 von 5 abgeschlossen
            </Badge>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="text-center mb-12">
          <div
            className={`inline-flex items-center justify-center w-24 h-24 rounded-full transition-all duration-700 ${
              showSuccess ? "bg-green-100 scale-100" : "bg-gray-100 scale-90"
            }`}
          >
            <CheckCircle
              className={`transition-all duration-700 ${showSuccess ? "w-12 h-12 text-green-600" : "w-10 h-10 text-gray-400"}`}
            />
          </div>
          <h2 className={`text-3xl font-bold mt-4 transition-opacity duration-700 ${showSuccess ? "opacity-100" : "opacity-0"}`}>
            {completedStep?.title} erfolgreich!
          </h2>
          <p
            className={`text-gray-600 mt-2 max-w-2xl mx-auto transition-opacity duration-700 delay-200 ${
        ...
        <div className="grid lg:grid-cols-3 gap-8 mb-8">
          ...
          <Card className="lg:col-span-2">
            <CardHeader>
              ...
              <CardDescription>Ein Schritt nach dem anderen. Das ist dein aktueller Stand.</CardDescription>
            </CardHeader>
            <CardContent>
          ...
          <div className="space-y-8">
            <Card className="bg-blue-50 border-blue-200">
              <CardHeader>
                <CardTitle className="text-blue-900">Sehr gut gemacht!</CardTitle>
              </CardHeader>
              <CardContent className="text-blue-800 space-y-4">
                <p>
                  Jedes Interview hilft uns, dein Startup besser zu verstehen. Die Ergebnisse bauen aufeinander auf und ergeben am Ende ein
                  Gesamtbild.
                </p>
                <p> Nimm dir kurz Zeit, um dich auf das nächste Thema vorzubereiten. Alle nötigen Informationen findest du auf der nächsten Seite. </p>
        ...
        {/* --- ElevenLabs Widget unten rechts (Standard) --- */}
        <elevenlabs-convai agent-id="UJcaKStMEPpXpzy9Uj5d"></elevenlabs-convai>
